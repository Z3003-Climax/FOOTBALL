...
const FinanceEngine = (() => {
  ...
  const STORAGE_KEY = 'eq_finance_v1';
  ...
  function initFinances(canon) {
    ...
    for (const [id, club] of Object.entries(canon.clubs)) {
      const tier   = club.tier || 'moyen';
      const budget = TIER_BUDGET[tier] || TIER_BUDGET.moyen;
      const squad  = Object.values(canon.players)
        .filter(p => p.club === id);
  ...
  function saveFinances(finances) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(finances)); } catch(e) {}
  }
  ...
  function clearFinances() { localStorage.removeItem(STORAGE_KEY); }
